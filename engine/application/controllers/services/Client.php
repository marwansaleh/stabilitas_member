...
class Client extends REST_Api {
    ...
    public function get_get($id){
        $this->load->model(array('rel_client_m'));
        $result = array('status'=>FALSE);
        
        $client = $this->rel_client_m->get($id);
        if ($client){
            $result['status'] = TRUE;
            $result['item'] = $client;
        }else{
    ...
    public function save_post(){
        ...
        if ($this->rel_client_m->get_count(array('fullname'=>$fullname))){
            $result['message'] = 'Nama Client yang sama sudah ada di database';
        }else{

            $id = $this->rel_client_m->save(array(
                'fullname'      => $fullname,
                'captive'       => $captive,
                'created'       => time()
            ), $this->post('id'));
            ...
            if ($id){
                $result['status'] = TRUE;
                $result['item'] = $this->rel_client_m->get($id);
            }else{
                $result['message'] = $this->rel_client_m->get_last_message();
            }
        }
        $this->response($result);
    }
    ...
    public function datatable_get(){
        ...
        $totalRecords = $this->rel_client_m->get_count();
        ...
        //get filtered count
        $totalFiltered = $this->rel_client_m->get_count();
        ...
        if ($totalRecords > 0){
            ...
            $query_result = $this->rel_client_m->get_by();
            if ($query_result){
    ...
    public function select2_get(){
        ...
        $query_result = $this->rel_client_m->get_offset('*',NULL,$start,$length);
    ...
    public function deletes_delete(){
        ...
        if ($ids && count($ids)){
            ...
            $messages = array();
            foreach ($ids as $id){
                $item = $this->rel_client_m->get($id);
                if ($this->rel_client_m->delete($id)){
                    $result['deleted'] [] = $item; 
                    $deleted_count++;
                }else{
    ...
    public function delete_delete($id){
        ...
        $item = $this->rel_client_m->get($id);
        if ($id && $this->rel_client_m->delete($id)){
            $result['status'] = TRUE;
            $result['item'] = $item;
        }else{
